...
export async function signInController (request: Request, response: Response): Promise<Response | undefined> {
  ...
  const isEmailValid: boolean = (mySqlResult != null)

  try {
    const authenticate = async () => {
      const { profilePassword } = request.body

      // @ts-expect-error isEmailValid determines mySqlResult will not be null
    ...
    return isEmailValid ? await authenticate() : response.json({ status: 400, data: null, message: 'Invalid email or password' })
  } catch (error: any) {
