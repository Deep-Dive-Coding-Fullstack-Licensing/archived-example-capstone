...
export async function selectAllTweets (): Promise<Tweet[]> {
  return  sql<Tweet[]>`SELECT tweet_id, tweet_profile_id, tweet_content, tweet_date FROM tweet ORDER BY tweet_date DESC`
}

export async function selectTweetByTweetId (tweetId: string): Promise<Tweet|null> {
  const result =  await sql<Tweet[]>`SELECT tweet_id, tweet_profile_id, tweet_content, tweet_date FROM tweet WHERE tweet_id = ${tweetId}`
  return result?.length === 1 ? result[0] : null
}

export async function selectTweetsByTweetProfileId (tweetProfileId: string): Promise<Tweet[]> {
  return  sql <Tweet[]> `SELECT tweet_id, tweet_profile_id, tweet_content, tweet_date FROM tweet WHERE tweet_profile_id = ${tweetProfileId}`
}